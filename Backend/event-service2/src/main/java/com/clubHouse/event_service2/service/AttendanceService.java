...
public class AttendanceService {
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Start attendance (with custom location/window)
    // FIX: evict EVENT_BY_ID so the next getEventById call returns fresh
    //      attendanceActive = true instead of the stale cached false value.
    // ─────────────────────────────────────────────────────────────────────────────
    @Transactional
    @Caching(evict = {
    ...
    public ApiResponse<QRCodeResponse> startAttendance(Long eventId,
        ...
        LocalDate windowStartDate = request.getAttendanceWindowStart().toLocalDate();
        if (windowStartDate.isEqual(LocalDate.now()) &&
                !request.getAttendanceWindowStart().isAfter(now)) {
            throw new InvalidOperationException(
                    "Attendance start time must be after the current time when scheduling for today"
            );
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Start attendance using pre-saved event location/window
    // FIX: same cache eviction — delegates to startAttendance which already evicts,
    //      but we annotate here too so Spring intercepts this method directly
    //      (Spring AOP only intercepts the outermost call, not internal this.calls).
    // ─────────────────────────────────────────────────────────────────────────────
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = CacheConfig.EVENT_BY_ID, key = "#eventId"),
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Get current QR code (read-only, no cache change needed)
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    public ApiResponse<AttendanceResponse> markAttendance(Long eventId,
        ...
        if (!isEnrolled) {
            ...
            return ApiResponse.error("NOT_ENROLLED", "You must be enrolled in this event to mark attendance", null);
        }

        if (!event.isAttendanceEnabled() || !event.isAttendanceActive()) {
            log.warn("❌ Attendance not active for event {}", eventId);
            return ApiResponse.error("ATTENDANCE_NOT_ACTIVE", "Attendance is not currently active for this event", null);
        }

        LocalDateTime now = LocalDateTime.now();
        if (now.isBefore(event.getAttendanceWindowStart())) {
            log.warn("❌ Attendance window not started for event {}", eventId);
            return ApiResponse.error("TIME_WINDOW_NOT_STARTED", "Attendance window has not started yet", null);
        }

        if (now.isAfter(event.getAttendanceWindowEnd())) {
            log.warn("❌ Attendance window closed for event {}", eventId);
            return ApiResponse.error("TIME_WINDOW_CLOSED", "Attendance window has closed", null);
        }
        ...
        if (!isQRValid) {
            ...
            return ApiResponse.error("INVALID_QR_CODE", "QR code is invalid or expired. Please scan the latest code.", null);
        }
        ...
        if (distance > event.getRadiusInMeters()) {
            ...
            return ApiResponse.error(
                    ...
                    String.format("You are %.0fm away from the event location. Required: within %dm", distance, event.getRadiusInMeters()),
                    null
            );
        }
        ...
        if (attendanceRepository.existsByEventAndPrn(event, prn)) {
            ...
            return ApiResponse.error("ALREADY_MARKED", "You have already marked attendance for this event", null);
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Stop attendance
    // FIX: evict caches so frontend immediately sees attendanceActive = false
    // ─────────────────────────────────────────────────────────────────────────────
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = CacheConfig.EVENT_BY_ID, key = "#eventId"),
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Read-only methods — no cache changes needed
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    public ApiResponse<AttendanceListResponse> getAttendanceList(Long eventId) {
        ...
        Double attendancePercentage = totalEnrolled > 0 ? (totalPresent * 100.0) / totalEnrolled : 0.0;
        ...
        return ApiResponse.success(String.format("Fetched attendance for %d students", totalPresent), response);
    }
    ...
    public ApiResponse<List<AttendanceResponse>> getMyAttendance(String prn) {
        ...
        return ApiResponse.success(String.format("Fetched %d attendance records", responses.size()), responses);
    }
    ...
    public ApiResponse<AttendanceResponse> checkMyAttendanceStatus(Long eventId, String prn) {
        ...
        Attendance attendance = attendanceRepository.findByEventAndPrn(event, prn).orElse(null);

        if (attendance == null) {
            AttendanceResponse response = AttendanceResponse.builder()
                    .success(false).eventId(eventId).prn(prn).message("Attendance not marked yet").build();
