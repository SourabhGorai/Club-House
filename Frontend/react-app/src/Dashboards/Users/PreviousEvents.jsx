 ...
 const fetchEvents = async (token, filter = "GLOBAL", targetId = null) => {
  try {
    ...
    if (response && response.data && response.data.success) {
      // The API already returns only completed events, no need to filter
      const completedEvents = response.data.data || [];
      
      // Fetch creator names for completed events
    ...
    setError(err.message || "An error occurred while fetching completed events");
  } finally {
    setLoading(false);
  }
};
