...
const MyEventsForSuperadmin = () => {
  ...
  const getFilteredEvents = () => {
    ...
    if (selectedDepartment !== 'all') {
      filtered = filtered.filter(event => {
        if (event.targetType?.toUpperCase() === 'DEPARTMENT' && event.targetIds) {
    ...
    if (selectedStatus !== 'all') {
      filtered = filtered.filter(event => 
        event.enrollmentStatus?.toLowerCase() === selectedStatus.toLowerCase()
      );
    }
  ...
  const closedEvents = events.filter(e => e.enrollmentStatus?.toLowerCase() === 'closed').length;
      ...
      <div className="relative max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white/80 backdrop-blur-sm p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-red-600">{closedEvents}</p>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            <div className="bg-blue-50/80 backdrop-blur-sm p-4 rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-bold text-blue-600">{globalEvents}</span>
              </div>
            </div>
            <div className="bg-purple-50/80 backdrop-blur-sm p-4 rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-bold text-purple-600">{clubEvents}</span>
              </div>
            </div>
            <div className="bg-green-50/80 backdrop-blur-sm p-4 rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-bold text-green-600">{departmentEvents}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 border border-white/20">
            ...
            {(selectedDepartment !== 'all' || selectedClub !== 'all' || selectedStatus !== 'all') && (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {selectedDepartment !== 'all' && (
                    <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm flex items-center">
                      ...
                      <button onClick={removeDepartmentFilter} className="ml-2 hover:text-green-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  ...
                  {selectedClub !== 'all' && (
                    <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm flex items-center">
                      ...
                      <button onClick={removeClubFilter} className="ml-2 hover:text-purple-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  ...
                  {selectedStatus !== 'all' && (
                    <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm flex items-center">
                      ...
                      <button onClick={removeStatusFilter} className="ml-2 hover:text-blue-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                ...
                >
                  ...
                  <div className={`relative h-48 bg-gradient-to-r ${categoryColor} p-6 overflow-hidden`}>
                    ...
                    <div className="absolute top-4 left-20">
                      <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1 ${targetTypeColor}`}>
                        {getTargetTypeIcon(event.targetType)}
                        <span className="ml-1">{event.targetType || 'N/A'}</span>
                      </span>
                    </div>
                    ...
                    <div className="absolute top-4 right-20 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button 
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="bg-gray-50 p-3 rounded-xl">
                        ...
                        <p className="text-xs text-gray-500 mt-1">PRN: {event.creatorPrn}</p>
                      </div>
                    </div>

                    {/* Department Info - Show if it's a department event */}
                    {event.targetType?.toUpperCase() === 'DEPARTMENT' && event.targetIds && event.targetIds.length > 0 && (
                      <div className="bg-green-50 p-3 rounded-xl">
                        <p className="text-xs text-gray-500 mb-2 flex items-center">
                          <Briefcase className="w-3 h-3 mr-1 text-green-600" />
                          Target Departments
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {event.targetIds.map(id => {
                            ...
                            return (
                              <span key={id} className="px-2 py-1 bg-green-100 text-green-700 rounded-lg text-xs font-medium">
                                {dept?.name || `Dept ID: ${id}`}
                              </span>
                            );
                          })}
                        </div>
                    ...
                    {/* Club Info - Show if it's a club event */}
                    {event.targetType?.toUpperCase() === 'CLUB' && event.targetIds && event.targetIds.length > 0 && (
                      <div className="bg-purple-50 p-3 rounded-xl">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {event.targetIds.map(id => {
                            ...
                            return (
                              <span key={id} className="px-2 py-1 bg-purple-100 text-purple-700 rounded-lg text-xs font-medium">
                                {club?.clubName || `Club ID: ${id}`}
                              </span>
                            );
                          })}
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-gray-500">
                        ...
                        <span>{event.maxEnrollments - event.currEnrollments} spots left</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-indigo-50 p-3 rounded-xl">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs text-indigo-600">QR Refresh: {event.qrRefreshInterval}s</span>
                      </div>
                      <p className="text-xs text-indigo-700">
                        {new Date(event.attendanceWindowStart).toLocaleTimeString()} - {new Date(event.attendanceWindowEnd).toLocaleTimeString()}
                      </p>
                    </div>
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center space-x-2 text-gray-500 text-sm">
            ...
            <span>Admin controls active • {filteredEvents.length} events displayed</span>
