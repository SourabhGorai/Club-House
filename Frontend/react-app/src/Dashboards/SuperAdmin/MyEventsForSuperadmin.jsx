...
/**
 * Central paged fetcher.
 * serverFilter:
 *   "all"               → GET /api/events/paged
 *   "enrollment:OPEN"   → GET /api/events/enrollment/OPEN/paged
 *   "enrollment:CLOSED" → GET /api/events/enrollment/CLOSED/paged
 *   "completed:true"    → GET /api/events/endEvent/true/paged
 *   "completed:false"   → GET /api/events/endEvent/false/paged
 */
...
//   // Function to fetch events by enrollment status
// const fetchEventsByEnrollmentStatus = async (status) => {
//   try {
//     setLoading(true);
//     const token = localStorage.getItem("token");
    
//     console.log(`Fetching ${status} events by enrollment status...`);
...
//     }
//   } catch (err) {
//     console.error(`Error fetching ${status} events by enrollment:`, err);
//     setError(err.message || "An error occurred while fetching events");
//   } finally {
//     setLoading(false);
//   }
...
// // Function to fetch events by completed status
...
//   // Since we're fetching filtered data from server,
//   // we only need to apply search and department/club filters
//   let filtered = [...events];

//   // Apply target type filter based on filterType
//   if (filterType === "GLOBAL") {
//     filtered = filtered.filter(event => event.targetType?.toUpperCase() === "GLOBAL");
//   } else if (filterType === "DEPARTMENT") {
//     filtered = filtered.filter(event => event.targetType?.toUpperCase() === "DEPARTMENT");
//   } else if (filterType === "CLUB") {
//     filtered = filtered.filter(event => event.targetType?.toUpperCase() === "CLUB");
//   }
...
//   // Apply department filter - only apply if not already filtered by DEPARTMENT type
//   if (selectedDepartment !== "all" && filterType !== "DEPARTMENT") {
...
//   // Apply club filter - only apply if not already filtered by CLUB type
//   if (selectedClub !== "all" && filterType !== "CLUB") {
//     filtered = filtered.filter((event) => {
...
//   // If clicking the same filter that's already active, go back to ALL
//   const updatedFilterType = filterType === newFilterType ? "ALL" : newFilterType;
  
//   setFilterType(updatedFilterType);
  
//   // Update UI states
//   setShowGlobalEvents(updatedFilterType === "GLOBAL");
//   setShowDepartmentEvents(updatedFilterType === "DEPARTMENT");
//   setShowClubEvents(updatedFilterType === "CLUB");
...
//   // Note: The actual filtering will happen in getFilteredEvents function
//   // based on the filterType value
// };
...
//   setShowDepartmentEvents(false);
//   setShowClubEvents(false);
//   const token = localStorage.getItem("token");
//   await fetchAllEvents(token);
// };
...
// const removeCompletedFilter = async () => {
//   setSelectedCompleted("all");
//   const token = localStorage.getItem("token");
//   await fetchAllEvents(token);
// };
...
// {/* Active Filters Display */}
// {(filterType !=="all" || selectedDepartment !== "all" ||
//   selectedClub !== "all" ||
//   selectedStatus !== "all" ||
//   selectedCompleted !== "all") && (
//   <div className="mt-4 pt-4 border-t border-gray-200">
...
//       {filterType !== "GLOBAL" && (
//         <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm flex items-center">
//           View: {filterType === "DEPARTMENT" ? "Department Events" : "Club Events"}
//           <button
...
//       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
//         {/* Department dropdown - show based on conditions */}
//         <div>
