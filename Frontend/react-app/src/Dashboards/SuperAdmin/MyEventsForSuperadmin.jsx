...
import {
  ...
  Filter, ChevronDown, Search, Settings, Edit, Trash2,
  Share2, Plus, Briefcase, X, ArrowLeft, ChevronLeft, ChevronRight,
  Map as MapIcon, Crosshair, Layers,
} from "lucide-react";
...
const MyEventsForSuperadmin = () => {
  ...
  const editCircleRef         = useRef(null);
  const editMapContainerRef   = useRef(null);
  const editMapInitializedRef = useRef(false);
  ...
  const [editSearchingLocation, setEditSearchingLocation] = useState(false);
  ...
  // ── Edit-modal map: initialize when modal opens, cleanup when it closes ──────
  useEffect(() => {
    if (showEditModal && editingEvent && editMapContainerRef.current && !editMapInitializedRef.current) {
    ...
    if (!showEditModal) {
      if (editMapRef.current) {
        ...
        editMapInitializedRef.current = false;
      }
    }
  }, [showEditModal, editingEvent]);

  // Update circle when radius field changes
  useEffect(() => {
    if (editCircleRef.current && editingEvent?.radiusInMeters) {
      editCircleRef.current.setRadius(parseInt(editingEvent.radiusInMeters) || 50);
    }
  }, [editingEvent?.radiusInMeters]);

  const initializeEditMap = () => {
    if (!editMapContainerRef.current || editMapInitializedRef.current) return;
    const lat = parseFloat(editingEvent?.latitude)  || 18.5204;
    const lng = parseFloat(editingEvent?.longitude) || 73.8567;
    const radius = parseInt(editingEvent?.radiusInMeters) || 50;
    try {
      editMapRef.current = L.map(editMapContainerRef.current).setView([lat, lng], 15);
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
        maxZoom: 19,
      }).addTo(editMapRef.current);
      editMarkerRef.current = L.marker([lat, lng], { draggable: true, autoPan: true }).addTo(editMapRef.current);
      editCircleRef.current = L.circle([lat, lng], {
        radius, color: "#4CA1AF", fillColor: "#4CA1AF", fillOpacity: 0.2, weight: 2,
      }).addTo(editMapRef.current);
      editMarkerRef.current.on("dragend", (e) => {
        const pos = e.target.getLatLng();
        updateEditCoordinates(pos.lat, pos.lng);
        editCircleRef.current?.setLatLng([pos.lat, pos.lng]);
      });
      editMapRef.current.on("click", (e) => {
        const { lat, lng } = e.latlng;
        editMarkerRef.current?.setLatLng([lat, lng]);
        editCircleRef.current?.setLatLng([lat, lng]);
        updateEditCoordinates(lat, lng);
      });
      setTimeout(() => editMapRef.current?.invalidateSize(), 100);
      editMapInitializedRef.current = true;
    } catch (err) {
      console.error("Error initializing edit map:", err);
    }
  };

  const updateEditCoordinates = (lat, lng) => {
    setEditingEvent((p) => ({ ...p, latitude: lat.toFixed(6), longitude: lng.toFixed(6) }));
  };

  const searchEditLocation = async () => {
    if (!editSearchQuery.trim()) return;
    try {
      setEditSearchingLocation(true);
      const res = await axios.get(
        `https://photon.komoot.io/api/?q=${encodeURIComponent(editSearchQuery)}&limit=1`
      );
      if (res.data?.features?.length > 0) {
        const [lon, lat] = res.data.features[0].geometry.coordinates;
        const latitude = parseFloat(lat), longitude = parseFloat(lon);
        editMapRef.current?.setView([latitude, longitude], 16);
        editMarkerRef.current?.setLatLng([latitude, longitude]);
        editCircleRef.current?.setLatLng([latitude, longitude]);
        updateEditCoordinates(latitude, longitude);
      }
    } catch (err) {
      console.error("Error searching location:", err);
    } finally {
      setEditSearchingLocation(false);
    }
  };
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
        ...
        {/* Edit Modal — now a standalone component with full map integration */}
        {showEditModal && editingEvent && (
          <EditEvent
            event={editingEvent}
            token={token}
            onClose={() => { setShowEditModal(false); setEditingEvent(null); }}
            onSuccess={() => { loadPage(serverFilter, currentPage); checkAllEventsAttendance(); }}
          />
        )}
