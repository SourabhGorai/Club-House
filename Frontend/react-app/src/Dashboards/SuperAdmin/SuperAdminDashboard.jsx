...
//   const [editingDept, setEditingDept] = useState(null); // null for create, {id, name} for edit
...
//               <div>
//                 <label className="block text-sm font-semibold text-gray-700 mb-2">PRN <span className="text-red-500">*</span></label>
//                 <input
//                   type="text" name="prn" value={profileData.prn} onChange={handleInputChange}
...
//                 <input
//                   type="text" name="department" value={profileData.department} onChange={handleInputChange}
//                   className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
...
//                 <input
//                   type="tel" name="phoneNumber" value={profileData.phoneNumber} onChange={handleInputChange}
//                   className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
...
//                 <button type="button" onClick={() => setShowProfileForm(false)} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-xl font-bold transition-all">Cancel</button>
//                 <button type="submit" disabled={profileLoading} className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-3 rounded-xl font-bold transition-all disabled:opacity-50">
...
//             <div className="bg-gradient-to-r from-purple-600 to-amber-600 p-8 text-white flex justify-between items-center">
//               <div>
//                 <h3 className="text-2xl font-bold tracking-tight">Department Management</h3>
//                 <p className="text-orange-50 text-sm opacity-90">Add or remove academic departments</p>
//               </div>
//               <button
//                 onClick={() => {
...
//               {/* Form to add/edit */}
//               <form onSubmit={handleDeptSubmit} className="mb-8 flex gap-3">
//                 <input
//                   type="text"
//                   placeholder="Enter department name..."
//                   className="flex-1 px-4 py-3 border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all outline-none"
//                   value={deptInput}
...
//                 <button
//                   type="submit"
//                   className="bg-purple-600 hover:bg-orange-700 text-white px-6 py-3 rounded-xl font-bold transition-all flex items-center gap-2 whitespace-nowrap shadow-lg shadow-orange-100"
//                 >
...
//               {/* List of departments */}
//               {deptLoading ? (
//                 <div className="py-10 text-center text-gray-500 italic">Updating list...</div>
//               ) : (
...
//                       <div 
//                         key={dept.id} 
//                         className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/30 transition-all group"
//                       >
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setDeptMessage({ text: response.data.message || "Failed to fetch departments", type: "error" });
      }
    } catch (error) {
      console.error("Error fetching departments:", error);
    ...
    try {
      if (editingDept) {
        ...
        if (response.data.success) {
          ...
          setDeptMessage({ text: response.data.message || "Failed to update department", type: "error" });
          return;
        }
      } else {
        ...
        if (response.data.success) {
          ...
          setDeptMessage({ text: response.data.message || "Failed to add department", type: "error" });
          return;
        }
      }
    ...
    if (window.confirm("Are you sure you want to delete this department?")) {
      try {
        ...
        if (response.data.success) {
          ...
          setDeptMessage({ text: response.data.message || "Failed to delete department", type: "error" });
        }
  ...
  const toggleDepartmentStatus = async (departmentId, currentStatus) => {
    try {
      ...
      if (response.data.success) {
        ...
        setDeptMessage({ text: response.data.message || "Failed to update status", type: "error" });
      }
    ...
    <div className="bg-purple-600 p-8 text-white flex justify-between items-center">
      <div>
            ...
            <div className="p-8 flex-1 overflow-y-auto">
              {/* Status message */}
              {deptMessage.text && (
                <div className={`mb-6 p-4 rounded-xl ${deptMessage.type === 'error' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
                    ...
                    departments.map((dept) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-gray-700">{dept.name}</span>
                          <span className={`text-xs font-bold px-2 py-1 rounded-full ${dept.active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
