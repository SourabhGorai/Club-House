...
export default function ManageClubs() {
  ...
  const fetchClubEventCounts = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/events/getEventCountForClub`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const countMap = response?.data?.data;
      if (response?.data?.success && countMap && typeof countMap === "object") {
        setClubEventCounts(countMap);
      } else {
        setClubEventCounts({});
      }
    } catch (err) {
      console.error("Error fetching club event counts:", err);
      setClubEventCounts({});
    }
  };
  ...
  const getClubEventCount = (clubId) => {
    if (clubId === null || clubId === undefined) return 0;
    const direct = clubEventCounts?.[clubId];
    if (typeof direct === "number") return direct;

    const asString = clubEventCounts?.[String(clubId)];
    const numericCount = Number(asString);
                  ...
                  <div className="space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          ...
                          >
                            ...
                            <span className="text-4xl font-black" style={{ color: theme.primaryColor }}>{getClubEventCount(selectedClub?.clubId)}</span>
                            <p className="text-xs font-bold uppercase" style={{ color: theme.textMuted }}>Organized Events</p>
                          </div>
                        </div>
