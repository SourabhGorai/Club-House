  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-70 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-neon-lg p-6 w-11/12 max-w-md transform transition-all duration-300 scale-100">
        ...
        {teacherSearchResult && (
          <div className={`mb-4 p-3 rounded-lg ${teacherSearchResult.role === 'TEACHERS' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            <p className="font-medium">{teacherSearchResult.username}</p>
            <p className="text-sm">Email: {teacherSearchResult.email}</p>
            <p className={`text-sm font-semibold ${teacherSearchResult.role === 'TEACHERS' ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium text-white rounded-full transition ${teacherSearchResult?.role === 'TEACHERS' && !assignTeacherLoading 
...
const handleAssignTeacher = async () => {
  if (!teacherSearchResult || !selectedClub || teacherSearchResult.role !== 'TEACHERS') {
    return;
  }
