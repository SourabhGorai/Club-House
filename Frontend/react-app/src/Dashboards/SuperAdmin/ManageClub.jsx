...
const handleActivateClub = async (clubId) => {
  try {
    const token = localStorage.getItem("token");
    // Try the activate endpoint
    const response = await axios.patch(
      `http://localhost:8080/api/clubs/${clubId}/activate`,
      {},
      {
        headers: { Authorization: `Bearer ${token}` }
      }
    ...
    // If activate endpoint fails, try updating the club directly
    try {
      const token = localStorage.getItem("token");
      ...
      alert("Failed to activate club. Please check the API endpoint.");
    }
  }
};
...
const handleSearchAdmin = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    // Try different possible endpoints for deactivating
    const response = await axios.patch(
      `http://localhost:8080/api/clubs/${clubToDelete.clubId}/deactivate`,
      {},
      {
        headers: { Authorization: `Bearer ${token}` }
      }
    ...
    // If the above fails, try updating the club directly
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
      ...
      console.error("Second attempt failed:", err2);
      alert("Failed to deactivate club. Please check the API endpoint.");
    }
  } finally {
    setDeleteLoading(false);
  }
...
<ConfirmationModal 
  ...
  message={`Deactivate "${clubToDelete?.clubName}"? The club will become inactive and assignments will be disabled.`} 
  onConfirm={handleDeleteClub} // Changed from handleDeleteClub
  onCancel={() => {
    setIsModalOpen(false);
    setClubToDelete(null);
  }} 
...
{selectedClub?.isActive ? (
  // Show deactivate button for active clubs
  <button 
    onClick={() => { 
      setClubToDelete(selectedClub); 
      setIsModalOpen(true); 
...
>
  ...
  <button 
    onClick={handleOpenClubAdminModal} 
    className={`text-[10px] uppercase px-4 py-2 mt-2 sm:mt-0 whitespace-nowrap rounded-full ${
      selectedClub?.isActive 
        ? "btn-gradient cursor-pointer" 
        : "bg-gray-300 text-gray-500 cursor-not-allowed"
    }`}
    disabled={!selectedClub?.isActive}
  >
    + Assign Admin
  </button>
)}
  ...
  <button 
    ...
    className={`text-[10px] uppercase px-4 py-2 rounded-full ${
      selectedClub?.isActive 
        ? "btn-gradient cursor-pointer" 
        : "bg-gray-300 text-gray-500 cursor-not-allowed"
    }`}
