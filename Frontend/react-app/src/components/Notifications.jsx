...
const parseBackendDate = (value) => {
  ...
  // dd-MM-yyyy HH:mm:ss (backend mapper format)
...
/**
 * FIX 2 — Convert any datetime string from the DB into "YYYY-MM-DDTHH:mm"
 * so DateTimePicker receives a value it understands.
 * DB stores "2026-03-20 13:00:00" (space separator, optional seconds).
 * DateTimePicker expects "2026-03-20T13:00".
 */
...
// FIX 2: toPickerValue() converts DB datetime → picker format
// ─────────────────────────────────────────────
const NotificationFormModal = ({ initial, onClose, onSubmit, saving }) => {
  ...
  const [form, setForm] = useState({
    ...
    // FIX: use toPickerValue so DB strings like "2026-03-20 13:00:00" are parsed correctly
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      // Append :00 seconds so Spring's LocalDateTime can deserialise it
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-7 py-5">
          <form id="notif-form" onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {form.targetedIds.map(id => (
                      <span key={id} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 rounded-full text-xs font-bold text-slate-600">
                        {id}
                        <button type="button" onClick={() => removeTargetId(id)} className="hover:text-red-500 cursor-pointer">
                          <X size={11} />
                        </button>
                      </span>
                    ))}
            ...
            {/* FIX 2: validUntil now pre-filled correctly via toPickerValue */}
...
// ─────────────────────────────────────────────
// Page Shell
// FIX 1: subtitle moved below title with mt-2 instead of mt-1 to prevent title clipping
// ─────────────────────────────────────────────
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#f8fafc" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* FIX 1: Added pb-2 on the title wrapper and mt-3 on the subtitle so the
            gradient text clip doesn't visually cut off descenders */}
...
const TeacherNotifications = () => {
  ...
  const askToggleActive = (notif) => setConfirmDialog({
    isOpen: true,
    title:       notif.isActive ? "Deactivate Notification" : "Reactivate Notification",
    message:     notif.isActive ? "This notification will stop being shown to users." : "This notification will become active and visible again.",
    confirmText: notif.isActive ? "Deactivate" : "Reactivate",
    variant:     notif.isActive ? "danger" : "primary",
    onConfirm:   async () => { closeConfirm(); await toggleActive(notif); },
  });
  ...
  const deleteNotif = async (notif) => {
    try {
      ...
      setToast({ msg: "Notification deleted", type: "success" }); fetchAll();
    } catch { setToast({ msg: "Failed to delete", type: "error" }); }
  };

  const askDelete = (notif) => setConfirmDialog({
    isOpen: true, title: "Delete Notification", message: "This action cannot be undone.",
    confirmText: "Delete", variant: "danger",
    onConfirm: async () => { closeConfirm(); await deleteNotif(notif); },
  });
      ...
      <div className="flex bg-white rounded-2xl border border-slate-200 shadow-sm p-1.5 mb-6 w-fit">
        {[["my", <Bell size={14} />, "My Notifications"], ["manage", <Zap size={14} />, "Manage"]].map(([t, ic, label]) => (
          <button key={t} onClick={() => { setTab(t); setPage(0); }}
            ...
            {ic}{label}
            {t === "my" && unread > 0 && (
              <span className="w-5 h-5 rounded-full bg-red-500 text-white text-[10px] font-black flex items-center justify-center">{unread}</span>
            )}
          </button>
        ))}
      </div>
            ...
            {filtered.length === 0
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {paginated.map(n => <AdminNotifRow key={n.notificationId} notif={n} onToggle={askToggleActive} onDelete={askDelete}
...
const SuperAdminNotifications = () => {
  ...
  const hardDelete = async (notif) => {
    try {
      await axios.delete(`${BASE_URL}/api/notification/${notif.notificationId}`, { headers: { Authorization: `Bearer ${token}` } });
      setToast({ msg: "Notification permanently deleted", type: "success" }); fetchAll();
    } catch { setToast({ msg: "Failed to delete", type: "error" }); }
  };

  const askHardDelete = (notif) => setConfirmDialog({
    isOpen: true, title: "Permanently Delete Notification",
    message: "This will permanently remove the notification and cannot be undone.",
    confirmText: "Delete Permanently", variant: "danger",
    onConfirm: async () => { closeConfirm(); await hardDelete(notif); },
  });
