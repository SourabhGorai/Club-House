...
const canManage    = (r) => isSuperAdmin(r) || isTeacher(r);
...
const TypeBadge = ({ type }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider" style={{ color: m.color, backgroundColor: m.bg }}>
      {m.icon}{m.label}
    </span>
  );
};
...
const EmptyState = ({ icon, title, subtitle, action }) => (
  <div className="flex flex-col items-center justify-center py-24 text-center px-4">
    <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-5" style={{ backgroundColor: "rgba(76,161,175,0.1)", color: "#4CA1AF" }}>
      {icon}
    </div>
...
const Loader = ({ text = "Loading notifications..." }) => (
  <div className="flex flex-col items-center justify-center py-24">
    <div className="w-12 h-12 border-4 rounded-full animate-spin mb-4" style={{ borderColor: "rgba(76,161,175,0.15)", borderTopColor: "#4CA1AF" }}></div>
      ...
      <div className="flex items-center gap-2">
        ...
        {items.map((item, idx) => item === "..." ? (
          ...
          <button key={item} onClick={() => onPage(item)}
            ...
            style={item === page ? { background: "linear-gradient(135deg, #4CA1AF, #315169)", color: "#fff", borderColor: "transparent" } : { backgroundColor: "#fff", borderColor: "#e2e8f0", color: "#64748b" }}>
            {item + 1}
          </button>
        ))}
...
// ─────────────────────────────────────────────
// Notification Card (user-facing read view)
// ─────────────────────────────────────────────
const NotificationCard = ({ notif, onMarkRead, onDelete, canDelete }) => (
  <div className={`group relative bg-white rounded-2xl border transition-all hover:shadow-lg ${!notif.isRead ? "border-l-4 shadow-sm" : "border-slate-100"}`}
    ...
    <div className="p-5">
      <div className="flex items-start gap-4">
        {/* Icon bubble */}
        <div className="flex-shrink-0 w-11 h-11 rounded-2xl flex items-center justify-center" style={{ backgroundColor: (TYPE_META[notif.notificationType] || TYPE_META.GLOBAL).bg, color: (TYPE_META[notif.notificationType] || TYPE_META.GLOBAL).color }}>
          {(TYPE_META[notif.notificationType] || TYPE_META.GLOBAL).icon && React.cloneElement((TYPE_META[notif.notificationType] || TYPE_META.GLOBAL).icon, { size: 20 })}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!notif.isRead && (
                <span className="w-2 h-2 rounded-full bg-[#4CA1AF] inline-block animate-pulse"></span>
              )}
            </div>
            <span className="text-[11px] text-slate-400 font-semibold whitespace-nowrap">{fmt(notif.createdAt)}</span>
          </div>

          <h4 className={`mt-2 font-black text-slate-800 leading-snug ${!notif.isRead ? "text-[#162F38]" : ""}`}>{notif.title}</h4>
      ...
      <div className="mt-4 flex items-center gap-2 justify-end">
        ...
        {canDelete && onDelete && (
          <button onClick={() => onDelete(notif.notificationId)}
            className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl text-red-500 hover:bg-red-50 transition-all cursor-pointer">
...
// ─────────────────────────────────────────────
// Admin Notification Row (table row for management)
// ─────────────────────────────────────────────
const AdminNotifRow = ({ notif, onToggle, onDelete, onEdit }) => (
  <tr className="group hover:bg-slate-50/80 transition-all border-b border-slate-100 last:border-0">
    ...
    <td className="px-4 py-5 hidden md:table-cell">
      <StatusDot active={notif.isActive} />
    </td>
    <td className="px-4 py-5 hidden lg:table-cell">
      <span className="text-xs font-semibold text-slate-400">{fmt(notif.createdAt)}</span>
    </td>
    <td className="px-4 py-5 hidden lg:table-cell">
      <span className="text-xs font-semibold text-slate-400">{fmt(notif.validUntil)}</span>
    </td>
...
// ─────────────────────────────────────────────
const NotificationFormModal = ({ initial, onClose, onSubmit, saving, departments, clubs }) => {
  ...
  const [form, setForm] = useState({
    ...
    validUntil: initial?.validUntil ? initial.validUntil.substring(0, 16) : "",
  });
  ...
  const addTargetId = () => {
    ...
    if (!isNaN(id) && !form.targetedIds.includes(id)) {
      set("targetedIds", [...form.targetedIds, id]);
      setTargetInput("");
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...form,
      sourceId: form.sourceId ? parseInt(form.sourceId) : undefined,
              ...
              <FormField label={`Target ${form.targetType} IDs`}>
                <div className="flex gap-2 mb-2">
                  ...
                  <button type="button" onClick={addTargetId}
                    className="px-4 py-3 rounded-2xl text-white text-sm font-bold cursor-pointer" style={{ background: "linear-gradient(135deg, #4CA1AF, #315169)" }}>
                    Add
                  </button>
                </div>
...
// ─────────────────────────────────────────────
// Page Shell (back nav + sticky header)
// ─────────────────────────────────────────────
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#f8fafc" }}>
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            <span className="hidden sm:flex items-center gap-2 text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full" style={{ color: "#4CA1AF", backgroundColor: "rgba(76,161,175,0.1)" }}>
              {icon}{roleLabel}
            </span>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight" style={{ background: "linear-gradient(135deg, #4CA1AF, #162F38)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              {title}
            </h1>
...
const UserNotifications = () => {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/api/notifications/me`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const markRead = async (id) => {
    try {
      await axios.patch(`${BASE_URL}/api/notifications/${id}/read`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const markAllRead = async () => {
    try {
      await axios.patch(`${BASE_URL}/api/notifications/me/read-all`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <PageShell
      ...
      {/* Reuse active toggle as "unread only" toggle via label trick */}
      ...
      {loading ? <Loader /> : filtered.length === 0 ? (
        ...
        <>
          <div className="space-y-3">
            {paginated.map(n => <NotificationCard key={n.notificationId} notif={n} onMarkRead={markRead} />)}
          </div>
...
const TeacherNotifications = () => {
  ...
  const [toast, setToast] = useState(null);
  const [tab, setTab] = useState("my"); // "my" | "manage"
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("");
  ...
  const fetchAll = useCallback(async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/notifications?active=${showActiveOnly}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const createOrUpdate = async (data) => {
    ...
    try {
      if (editNotif) {
        await axios.patch(`${BASE_URL}/api/notifications/${editNotif.notificationId}`,
        ...
        await axios.post(`${BASE_URL}/api/notifications`, data, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const toggleActive = async (notif) => {
    try {
      const ep = notif.isActive ? "deactivate" : "reactivate";
      await axios.patch(`${BASE_URL}/api/notifications/${notif.notificationId}/${ep}`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const askToggleActive = (notif) => {
    const isDeactivate = !!notif.isActive;
    setConfirmDialog({
      isOpen: true,
      title: isDeactivate ? "Deactivate Notification" : "Reactivate Notification",
      message: isDeactivate
        ? "This notification will stop being shown to users."
        : "This notification will become active and visible again.",
      confirmText: isDeactivate ? "Deactivate" : "Reactivate",
      variant: isDeactivate ? "danger" : "primary",
      onConfirm: async () => {
        closeConfirm();
        await toggleActive(notif);
      },
    });
  };
  ...
  const deleteNotif = async (notif) => {
    try {
      await axios.delete(`${BASE_URL}/api/notifications/${notif.notificationId}`, { headers: { Authorization: `Bearer ${token}` } });
      setToast({ msg: "Notification deleted", type: "success" });
      fetchAll();
    } catch { setToast({ msg: "Failed to delete", type: "error" }); }
  };

  const askDeleteNotif = (notif) => {
    setConfirmDialog({
      isOpen: true,
    ...
    <PageShell title="Notifications" subtitle="View your notifications and manage club announcements"
      ...
      {(showForm || editNotif) && (
        <NotificationFormModal initial={editNotif} onClose={() => { setShowForm(false); setEditNotif(null); }}
          onSubmit={createOrUpdate} saving={saving} />
      )}
      <ConfirmDialog
      ...
      {/* Tabs */}
      <div className="flex bg-white rounded-2xl border border-slate-200 shadow-sm p-1.5 mb-6 w-fit">
        {[["my", <Bell size={14} />, "My Notifications"], ["manage", <Zap size={14} />, "Manage"]].map(([t, ic, label]) => (
          <button key={t} onClick={() => { setTab(t); setPage(0); }}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all cursor-pointer"
            style={tab === t ? { background: "linear-gradient(135deg, #4CA1AF, #315169)", color: "#fff" } : { color: "#64748b" }}>
            {ic}{label}{t === "my" && unread > 0 && <span className="w-5 h-5 rounded-full bg-red-500 text-white text-[10px] font-black flex items-center justify-center">{unread}</span>}
          </button>
        ))}
      </div>
      ...
      <FilterBar search={search} onSearch={v => { setSearch(v); setPage(0); }}
        typeFilter={typeFilter} onType={v => { setTypeFilter(v); setPage(0); }}
        showActive={showActiveOnly} onActive={v => { setShowActiveOnly(v); setPage(0); }}
        showActiveToggle={tab === "manage"} />
      ...
      {loading ? <Loader /> : tab === "my" ? (
        ...
        <>
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            {filtered.length === 0 ? (
              <EmptyState icon={<BellOff size={36} />} title="No notifications found" subtitle="Create a new notification to get started."
                action={<button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-5 py-3 rounded-2xl text-white text-sm font-bold cursor-pointer" style={{ background: "linear-gradient(135deg, #4CA1AF, #315169)" }}><Plus size={15} /> Create First</button>} />
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {paginated.map(n => <AdminNotifRow key={n.notificationId} notif={n} onToggle={askToggleActive} onDelete={askDeleteNotif}
                      onEdit={n2 => { setEditNotif(n2); setShowForm(true); }} />)}
                  </tbody>
                </table>
              </div>
            )}
...
const SuperAdminNotifications = () => {
  ...
  const toggleActive = async (notif) => {
    try {
      await axios.patch(`${BASE_URL}/api/notifications/${notif.notificationId}/${notif.isActive ? "deactivate" : "reactivate"}`, {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const askHardDelete = (notif) => {
    setConfirmDialog({
      isOpen: true,
      title: "Permanently Delete Notification",
      message: "This will permanently remove the notification and cannot be undone.",
      confirmText: "Delete Permanently",
      variant: "danger",
      onConfirm: async () => {
        closeConfirm();
        await hardDelete(notif);
      },
    });
  };
  ...
  return (
    <PageShell title="Notification Management" subtitle="System-wide notification control center — create, manage, and broadcast"
      ...
      <StatsRow stats={[
        { label: "Total",    value: notifications.length,                          icon: <Bell size={20} />,     color: "#4CA1AF", bg: "rgba(76,161,175,0.1)" },
        { label: "Active",   value: notifications.filter(n => n.isActive).length,  icon: <Eye size={20} />,      color: "#10B981", bg: "rgba(16,185,129,0.1)" },
        { label: "Inactive", value: notifications.filter(n => !n.isActive).length, icon: <EyeOff size={20} />,   color: "#F97316", bg: "rgba(249,115,22,0.1)" },
        { label: "Types",    value: [...new Set(notifications.map(n => n.notificationType))].length, icon: <Layers size={20} />, color: "#8B5CF6", bg: "rgba(139,92,246,0.1)" },
      ]} />
      ...
      {loading ? <Loader /> : (
        <>
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            {filtered.length === 0 ? (
              <EmptyState icon={<BellOff size={36} />} title="No notifications found"
                subtitle="Create your first notification to reach your users."
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {paginated.map(n => (
                      <tr key={n.notificationId} className={`group border-b border-slate-100 last:border-0 transition-all ${selectedIds.has(n.notificationId) ? "bg-[rgba(76,161,175,0.04)]" : "hover:bg-slate-50/80"}`}>
...
// ─────────────────────────────────────────────
// Root: Route to the right component by role
// ─────────────────────────────────────────────
const Notifications = () => {
  const role = getRole();
