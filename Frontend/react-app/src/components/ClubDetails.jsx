  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
      ...
      >
        ...
        {/* Scrollable list */}
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 theme-scrollbar">
          {loading ? (
            <div className="text-center py-8">
              <div
                className="animate-spin rounded-full h-8 w-8 border-b-2 mx-auto"
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p
          className="text-sm font-semibold break-all leading-snug"
          style={{ color: theme?.textPrimary }}
        >
          {value}
        </p>
...
const AvatarPanel = ({
  ...
  >
    <div
      className="w-20 h-20 rounded-full overflow-hidden border-4 border-white shadow-lg ring-2"
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-5 min-w-0">
            ...
            <DetailRow
              icon={GraduationCap}
              label="Department"
              value={adminData.teacherDepartment}
              iconColor="#10B981"
              bgColor="#ECFDF5"
              theme={theme}
            />
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-5 min-w-0">
            ...
            <DetailRow
              icon={Mail}
              label="Email"
              value={admin.email !== "N/A" ? admin.email : null}
              iconColor="#F59E42"
              bgColor="#FFF7ED"
              theme={theme}
            />
            ...
            <DetailRow
              icon={BookOpen}
              label="Year"
              value={admin.year ? `Year ${admin.year}` : null}
              iconColor="#6366F1"
              bgColor="#EEF2FF"
              theme={theme}
            />
...
export default function ClubDetails() {
  ...
  const fetchProfileImages = async (membersList) => {
    const withImages = membersList.filter(
      (m) => m.hasProfileImage && m.imageUrl,
    );
    const results = await Promise.all(
      withImages.map(async (member) => {
        try {
  ...
  const fetchEventCounts = async (clubId, departmentName) => {
    try {
      ...
      if (
        clubResponse.data &&
        clubResponse.data.success &&
        Array.isArray(clubResponse.data.data)
      ) {
        const events = clubResponse.data.data;
        const upcoming = events.filter((event) => !event.completed);
        const past = events.filter((event) => event.completed);
      ...
      if (departmentName) {
        try {
          ...
          if (
            ...
            Array.isArray(departmentResponse.data.data)
          ) {
            const deptEvents = departmentResponse.data.data;
            const deptUpcoming = deptEvents.filter((event) => !event.completed);
            const deptPast = deptEvents.filter((event) => event.completed);
  ...
  const fetchEventCountsFallback = async (clubId) => {
    try {
      ...
      if (upcomingRes?.data?.success) {
        const totalElements =
          upcomingRes.data.data?.totalElements ||
          upcomingRes.data.data?.length ||
          (Array.isArray(upcomingRes.data) ? upcomingRes.data.length : 0);
        setUpcomingCount(totalElements);
      }
      ...
      if (previousRes?.data?.success) {
        const totalElements =
          previousRes.data.data?.totalElements ||
          previousRes.data.data?.length ||
          (Array.isArray(previousRes.data) ? previousRes.data.length : 0);
        setPreviousCount(totalElements);
      }
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 sm:gap-6">
            <div className="flex items-start sm:items-center gap-3 sm:gap-6 min-w-0">
              ...
              <div className="min-w-0">
                <h1
                  className="text-2xl sm:text-4xl font-bold mb-2 break-words"
                  style={{ color: theme.textPrimary }}
                >
                  {clubDetails.clubName}
                </h1>
                <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                  ...
                  <span
                    ...
                    style={{ color: theme.textSecondary }}
                  >
                    <Calendar size={18} style={{ color: "#6366F1" }} /> Added{" "}
                    {formatDate(clubDetails.createdAt)}
                  </span>
                  <div
                    className={`px-2 py-1 rounded-md flex items-center gap-1.5`}
                    style={{
                      background: clubDetails.isActive
                        ? "rgba(16,185,129,0.1)"
                        : theme.accentSoft,
                    }}
                  >
